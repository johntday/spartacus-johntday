...
import { CmsService, isNotUndefined, LoggerService, Page, UnifiedInjector } from '@spartacus/core';
...
export class ContentfulPageLayoutService implements OnDestroy {
  ...
  getPageFoldSlot(pageTemplate: string): Observable<string | undefined> {
    return this.breakpointService.breakpoint$.pipe(
      map((breakpoint) => {
        ...
        const config = this.getResponsiveSlotConfig(<LayoutSlotConfig>pageTemplateConfig, 'pageFold', breakpoint);
  ...
  protected getSlotConfig(templateUid: string, configAttribute: string, section?: string, breakpoint?: BREAKPOINT): SlotConfig | undefined {
    ...
    if (pageTemplateConfig) {
      return this.getResponsiveSlotConfig(<LayoutSlotConfig>pageTemplateConfig, configAttribute, breakpoint);
    }
    return undefined;
  }
  ...
  protected getSlotConfigForSection(templateUid: string, configAttribute: string, section?: string, breakpoint?: BREAKPOINT): SlotConfig | undefined {
    ...
    const responsiveConfig = this.getResponsiveSlotConfig(<LayoutSlotConfig>sectionConfig, configAttribute, breakpoint);
